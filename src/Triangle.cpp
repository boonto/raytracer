...
Primitive::IntersectionA Triangle::intersect(const Ray &ray, const float dist) const {
    ...
    if (t < dist && t > std::numeric_limits<float>::epsilon()) {
        return IntersectionA{true, t};
    }

    return IntersectionA{false, 0};
}

Primitive::IntersectionB Triangle::getIntersectionVectors(const Ray &ray, const float dist) const {
    auto edge0 = vertices[1] - vertices[0];
    auto edge1 = vertices[2] - vertices[0];
    ...
    auto det = glm::dot(edge0, P);
    if(abs(det) < std::numeric_limits<float>::epsilon())
        return IntersectionB{glm::vec3{0.0f}, glm::vec3{0.0f}};
